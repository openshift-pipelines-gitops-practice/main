...
| リポジトリ | 役割 | 管理者 |
|-----------|------|--------|
| **本リポジトリ** (メイン) | Pipeline 定義・PaC 設定・Argo CD リソースなどのプラットフォーム資材 | プラットフォームチーム |
...
| **Manifest リポジトリ** (app-manifests) | Kustomize マニフェスト（Deployment / Service / Route）。イメージタグは CI が自動更新 | アプリチーム |
...
| `dev-handson-app` | Argo CD Application / デプロイされるアプリケーション |
...
| ステップ | 内容 | 使用 Task |
|---------|------|-----------|
| **fetch-source** | ソースコードを clone | `git-clone` |
| **unit-test** | テスト実行 | `maven` (Backend) / インライン npm (Frontend) |
| **build-image** | S2I でコンテナイメージをビルド（タグ = コミットハッシュ） | `s2i-java` / `s2i-nodejs` |
| **fetch-manifest** | Manifest リポジトリを clone | `git-clone` |
| **update-manifest** | `kustomization.yaml` のイメージタグを書き換えて push | `git-cli` |
| **sync-argocd** | Argo CD に同期を指示 | `argocd-task-sync-and-wait` |

使用する Tekton Task はすべて OpenShift Pipelines が提供するプリセット Task（`openshift-pipelines` Namespace）です。
...
┌─────────────────────────────────────────────────────────────────────┐
│  プラットフォームチーム                                              │
│  ・Pipeline 定義の作成・メンテナンス (pipelines/)                    │
...
- Git リポジトリが 3 つ用意されていること（Backend / Frontend / Manifest）
...
3. Pipeline リソースのデプロイ
4. PaC Repository の登録
5. ソースリポジトリに `.tekton/pipelinerun.yaml` を配置
6. Manifest リポジトリの準備
